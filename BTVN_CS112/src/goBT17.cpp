...
struct RegionStat {
    ...
    int countOther = 0;
    float ratio36() const {
        int total = count36 + countOther;
        return total == 0 ? 0.0f : (float)count36 / total;
    }
};

pair<int,int> getRegionID(float x, float y) {
    int gx = floor(x / 300.0f);
    int gy = floor(y / 300.0f);
    return {gx, gy};
}

Point getNextExplorationPoint(float x, float y) {
    static float dir = 0; 
    dir += M_PI / 3;
    float step = 200.0f;
    return {x + step * cos(dir), y + step * sin(dir)};
}

Point getRandomPointAround(Point center, float radius = 400.0f) {
...
inline pair<float,float> Module::T1(float x, float y, int lastBossID) {
	static bool foundSpecial = false;
    static Point specialCenter;
    static map<pair<int,int>, RegionStat> stats;

    auto id = getRegionID(x, y);
    if (lastBossID == 36) stats[id].count36++;
    else if (lastBossID != 0) stats[id].countOther++;

    if (!foundSpecial) {
        for (auto &it : stats) {
            if (it.second.ratio36() > 0.45f) {
                foundSpecial = true;
                specialCenter = {it.first.first * 300.0f, it.first.second * 300.0f};
                break;
            }
        }
    }
    ...
    Point next;
    if (!foundSpecial) {
        next = getNextExplorationPoint(x, y);
    } else {
        auto rid = getRegionID(specialCenter.x, specialCenter.y);
        if (stats[rid].ratio36() < 0.3f) {
            foundSpecial = false;
            next = getNextExplorationPoint(x, y);
        } else {
            next = getRandomPointAround(specialCenter, 400.0f);
        }
    }
