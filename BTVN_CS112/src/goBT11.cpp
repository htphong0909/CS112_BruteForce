...
inline pair<float,float> Module::T1(float x, float y, int lastBossID) {
	static float x_bandau=x,y_bandau=y;
    static bool inSpecialZone=0;
    static float ix,iy; //tam cua khu vuc dac biet
    static float goc1=0,goc2=0;
    static float heso1=pi/4,heso2=pi/4;
    static int t1=50,t2=50;
    if(lastBossID==36) { //nếu boss cũ là 36 thì tui chắc chắn đã vô vùng đặc biệt
        ix=x,iy=y; 
        inSpecialZone=1;
        t1=50;
        goc1=0;
    ...
    if(inSpecialZone){
        ...
        if(goc1>=2*pi){
            goc1=0;
            heso1/=2;  // sau mỗi vòng quét thì ta nới rộng bán kính ra thêm 50
            t1+=50;   // đồng thời giảm hệ số góc đi phân nửa
        }
        return {x,y};
    }
    else{
        x=x_bandau+t2*cos(goc2); //phần này tương tự, ta quét đường tròn dựa vào tâm là điểm xuất phát ban đầu
        y=y_bandau+t2*sin(goc2);
        goc2+=heso2;
        if(goc2>=2*pi){
            goc2=0;
            heso2/=2;
