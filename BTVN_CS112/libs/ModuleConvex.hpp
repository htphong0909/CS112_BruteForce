...
namespace Module
{
    ...
    static uint32_t offsetSpiral = 100;
    static uint32_t offsetBacktrack = 0;
    static uint32_t bossLevel = 4;
    static uint32_t tpStep = 4;
    static uint32_t mapWidth = 4300;
    static uint32_t entityScandSpiral = 30;
    static uint32_t entityScand = entityScandSpiral * 10;
    ...
    void GetNextPointSpiral(float &x, float &z, std::unordered_set<std::string> &visitedPoints, bool recenter = false);
...
namespace Module
{
        ...
        while (functionRunMap[{pid, "FollowTarget"}].load())
        {
            ...
            if (scanAll)
            {
                ...
                if (goSpiral != 0) {
                    ...
                    targetZ = game.data.player.data.coord.data.z.UpdateData().data, visitedPoints, goSpiral >= offsetSpiral);
                }
                else GetNextPoint(
                    targetX = game.data.player.data.coord.data.x.UpdateData().data,
            ...
            if (!player && !target) {
                if (goSpiral > 0) goSpiral--;
                continue;
            }
            else if (target)
            {
                if (std::regex_match(target->data.name.data, std::regex(".*giantlands.*"))) {
                    goSpiral = offsetSpiral;
                    backtrackCnt = 0;
                }
                else if (goSpiral > 0) goSpiral--;
    ...
    std::unique_ptr<Game::World::Entity> FindTarget(Game &game, const bool &targetBoss, const bool &targetPlant, const bool &targetNormal, const std::vector<std::string> &targets, const std::vector<std::string> &noTargets, const uint32_t &range, const std::vector<Target> &blackTargetList)
    {
        ...
        for (auto &entity : entitys)
        {
            ...
            if (std::regex_match(currentTarget.name, std::regex(".*npc.*")))
            {
                ...
                {
                    if (!std::regex_match(currentTarget.name, std::regex(".*giantlands.*")) 
                    && !std::regex_match(currentTarget.name, std::regex(".*shadow.*"))) continue;

                    if (currentTarget.level >= 10 && currentTarget.health >= 400000000) continue;
    ...
    // ====== Convex Hull utils (Monotone Chain) ======
    struct Pt {           // giữ nguyên kiểu vào/ra của bạn (double)
        double x, y;
    };

    struct IPt {          // điểm nguyên ổn định
        long long x, y;
        bool operator<(const IPt& o) const {
    ...
    // quy đổi double -> integer theo lưới (llround)
    static inline IPt toIPt(const Pt& p) {
        return IPt{ std::llround(p.x), std::llround(p.y) };
    }
    static inline IPt toIPt(double x, double y) {
        return IPt{ std::llround(x), std::llround(y) };
    }

    static inline __int128 cross_i(const IPt& O, const IPt& A, const IPt& B) {
        // (A - O) x (B - O)
        return (__int128)(A.x - O.x) * (B.y - O.y) - (__int128)(A.y - O.y) * (B.x - O.x);
    }

    static std::vector<IPt> convex_hull_i(std::vector<IPt> pts) {
        if (pts.size() <= 1) return pts;
        // sort + unique (trên số nguyên, ổn định tuyệt đối)
        std::sort(pts.begin(), pts.end());
        pts.erase(std::unique(pts.begin(), pts.end()), pts.end());

        int n = (int)pts.size(), k = 0;
        std::vector<IPt> H(2*n);

        // lower hull
        for (int i = 0; i < n; ++i) {
            while (k >= 2 && cross_i(H[k-2], H[k-1], pts[i]) <= 0) --k;
            H[k++] = pts[i];
        }
        // upper hull
        for (int i = n-2, t = k+1; i >= 0; --i) {
            while (k >= t && cross_i(H[k-2], H[k-1], pts[i]) <= 0) --k;
            H[k++] = pts[i];
        }
    ...
    // trả về 2*diện tích đa giác (không âm) dưới dạng long double để so sánh
    static inline long double poly_area2_i(const std::vector<IPt>& P) {
        __int128 s = 0;
        int n = (int)P.size();
        for (int i = 0, j = n - 1; i < n; j = i++) {
            s += (__int128)P[j].x * P[i].y - (__int128)P[j].y * P[i].x;
        }
        if (s < 0) s = -s;
        // cast sang long double để dùng ngưỡng tương đối/ tuyệt đối linh hoạt
        return (long double)s;
    }

    // ====== Trạng thái bao lồi động của các CENTER (dùng số nguyên) ======
    static std::vector<IPt> g_centers_i;       // tất cả tâm đã chấp nhận (int)
    static std::vector<IPt> g_hull_i;          // bao lồi hiện tại (int)
    static std::unordered_set<std::string> g_centerKeys; // chống trùng theo khóa chuỗi

    static inline std::string keyCenterLL(long long X, long long Y) {
        return std::to_string(X) + "|" + std::to_string(Y);
    }
    static inline std::string keyCenter(double X, double Y) { // tương thích chỗ gọi cũ nếu có
        return keyCenterLL(std::llround(X), std::llround(Y));
    }

    // Kiểm tra "cải thiện" bao lồi (dùng integer hull)
    // Giữ nguyên chữ ký để bạn không phải đổi các chỗ gọi.
    static bool improves_hull_if_added(const Pt& p, double abs_eps = 1.0L, long double rel_eps = 1e-12L) {
        IPt ip = toIPt(p);
        // nếu đã có tâm trùng (sau llround) thì không cải thiện
        if (g_centerKeys.count(keyCenterLL(ip.x, ip.y))) return false;

        if (g_centers_i.size() < 2) return true; // 0-1 điểm: luôn chấp nhận

        // hull mới sau khi thêm ip
        std::vector<IPt> cand = g_centers_i;
        cand.push_back(ip);
        auto newHull = convex_hull_i(std::move(cand));

        if (g_hull_i.empty()) return true;

        long double oldA2 = poly_area2_i(g_hull_i);
        long double newA2 = poly_area2_i(newHull);

        // cải thiện nếu diện tích tăng đáng kể (relative/absolute) hoặc số đỉnh tăng
        if (newA2 > oldA2 * (1.0L + rel_eps) + (long double)abs_eps) return true;
        if ((int)newHull.size() > (int)g_hull_i.size()) return true;
        return false;
    }

    // Chấp nhận center & cập nhật hull (integer)
    static void accept_center_and_update_hull(const Pt& p) {
        IPt ip = toIPt(p);
        std::string k = keyCenterLL(ip.x, ip.y);
        if (!g_centerKeys.count(k)) {
            g_centerKeys.insert(k);
            g_centers_i.push_back(ip);
        }
        g_hull_i = convex_hull_i(g_centers_i);
    }


    void GetNextPointSpiral(float &x, float &z,
                        std::unordered_set<std::string> &visitedPoints,
                        bool recenter)
    {
        const float sqrt3 = std::sqrt(3.0f);
        const float e = entityScandSpiral;               // bước lưới hex
        const int   R = static_cast<int>(mapWidth / (2*e));

        // ===== GIÃN CÁCH VÒNG THEO HÀM MŨ =====
        const int    RING_BASE   = 1;     // bán kính vòng đầu tiên (đơn vị "step axial")
        const double RING_GROWTH = 2;   // hệ số tăng bán kính hiệu dụng (>1.0)
        auto ringEffRaw = [&](int ring)->int {
            double val = (double)RING_BASE * std::pow(RING_GROWTH, std::max(0, ring-1));
            int rEff = (int)std::round(val);
            rEff = std::max(1, std::min(rEff, R));
            return rEff;
        };

        // ===== CHUYỂN ĐỔI (x,z) <-> (i,j) axial =====
        auto alignToGrid = [&](float &X, float &Z, int &i, int &j) {
            ...
            auto clampIndex = [&](int &I, int &J) {
                ...
                J = std::clamp(J, -R, R);
                while (I*I + I*J + J*J > R*R) {
                    if (std::abs(I) > std::abs(J)) I += (I > 0 ? -1 : 1);
                    else                           J += (J > 0 ? -1 : 1);
                }
            };
            clampIndex(i, j);
        ...
        auto ij_to_xz = [&](int i, int j, float &X, float &Z) {
            X = e * (2 * i + j);
            Z = e * sqrt3 * j;
        };
        auto keyIJ = [&](int i, int j) {
            float X, Z; ij_to_xz(i, j, X, Z);
            return std::to_string((int)std::round(X)) + "|" + std::to_string((int)std::round(Z));
        };

        // ===== HƯỚNG AXIAL CHUẨN (CCW): E, SE, SW, W, NW, NE =====
        const int D[6][2] = {
            {+1,  0}, {+1, -1}, { 0, -1}, {-1,  0}, {-1, +1}, { 0, +1}
        };

        // ===== TRẠNG THÁI XOẮN (lưu tĩnh) =====
        struct SpiralState {
            bool init = false;
            int  ic = 0, jc = 0;   // tâm hiện tại
            int  ring = 1;         // ring logic hiện tại (>=1)
            int  side = 0;         // cạnh 0..5
            int  step = 0;         // bước đã đi trên cạnh (0..rEff-1)
            int  ci = 0, cj = 0;   // điểm xoắn hiện tại (đã phát)
            bool atCorner = true;  // cần phát góc mở đầu vòng
            int  prevREff = 0;     // rEff trước đó (để ép tăng tối thiểu +1)
        };
        static SpiralState S;

        // ===== SNAP VỊ TRÍ HIỆN TẠI =====
        int i_cur, j_cur;
        alignToGrid(x, z, i_cur, j_cur);

        // ===== KHỞI TẠO =====
        if (!S.init) {
            S.init = true;
            S.ic = i_cur; S.jc = j_cur;
            S.ring = 1; S.side = 0; S.step = 0; S.atCorner = true;
            S.prevREff = 0;

            // Thêm tâm đầu tiên vào bao lồi theo integer
            float X0, Z0; ij_to_xz(S.ic, S.jc, X0, Z0);
            Pt p0{ (double)X0, (double)Z0 };
            accept_center_and_update_hull(p0);   // <- dùng hàm integer-safe
        }
        std::cout << "new target = " << ((recenter) ? "true" : "false") << '\n';


        // ===== RECENTER =====
        if (recenter) {
            // Candidate center = vị trí snap hiện tại
            float Xc, Zc; ij_to_xz(i_cur, j_cur, Xc, Zc);
            Pt cand{ (double)Xc, (double)Zc };

            // Chỉ chấp nhận nếu cải thiện bao lồi (so sánh trên số nguyên)
            if (improves_hull_if_added(cand)) {

                std::cout << "recenter = true\n";

                accept_center_and_update_hull(cand);   // cập nhật hull (integer)

                // Reset spiral quanh tâm mới (axial)
                S.ic = i_cur; S.jc = j_cur;
                S.ring = 1; S.side = 0; S.step = 0; S.atCorner = true;
                S.prevREff = 0;
            }
            else std::cout << "recenter = false\n";
        }
        else std::cout << "recenter = false\n";

        // đánh dấu ô hiện tại (tuỳ mục đích)
        visitedPoints.insert(keyIJ(i_cur, j_cur));

        // rEff theo hàm mũ, đảm bảo tăng tối thiểu +1 so với lần trước
        auto ringEff = [&](int ring)->int {
            int rEff = ringEffRaw(ring);
            rEff = std::max(rEff, S.prevREff + 1);
            rEff = std::min(rEff, R);
            return rEff;
        };
        ...
        // ===== PHÁT 1 BƯỚC XOẮN =====
        auto advance_one = [&](){
            int rEff = ringEff(S.ring);

            if (S.atCorner) {
                // Góc mở đầu ở hướng NW của ring hiện tại (pattern chuẩn, CCW)
                S.ci = S.ic + rEff * D[4][0];
                S.cj = S.jc + rEff * D[4][1];
                S.side = 0;
                S.step = 0;
                S.atCorner = false;
                return;
            ...
            if (S.step >= rEff) {
                ...
                if (S.side == 0) {
                    ...
                    S.prevREff = rEff;
                    S.ring++;
                    // Hành vi ở biên: bò mép (giữ tại biên)
                    if (ringEff(S.ring) >= R) {
                        S.prevREff = R;
                        // ring logic vẫn tăng được nhưng rEff kẹp ở R, tiếp tục đi vòng biên
                    }
                    S.atCorner = true;
                }
            }
        ...
        // Bỏ qua các điểm ngoài biên hex cho đến khi hợp lệ
        int guard = 0;
        do {
            advance_one();
            if (++guard > 12*R + 24) break;
        } while (S.ci*S.ci + S.ci*S.cj + S.cj*S.cj > R*R);

        float X, Z; ij_to_xz(S.ci, S.cj, X, Z);
        x = X; z = Z;
        visitedPoints.insert(keyIJ(S.ci, S.cj));

        std::cout << "current center: " << S.ic << " " << S.jc << "\nnext point: " << S.ci << " " << S.cj << "\ncurrent ring: " << S.ring << "\n"; 
    }
