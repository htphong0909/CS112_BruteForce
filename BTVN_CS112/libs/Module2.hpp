...
namespace Module
{
    ...
    static uint32_t offsetSpiral = 300;
    static uint32_t offsetBacktrack = 0;
    static uint32_t bossLevel = 4;
    static uint32_t tpStep = 4;
    static uint32_t mapWidth = 4300;
    static uint32_t entityScandSpiral = 15;
    static uint32_t entityScand = entityScandSpiral * 15;
    ...
    void GetNextPointSpiral(float &x, float &z, std::unordered_set<std::string> &visitedPoints, bool recenter = false, bool goBacktrack = false);
...
namespace Module
{
    ...
    void FollowTarget(const Memory::DWORD &pid, const std::vector<std::string> &players, const std::vector<std::string> &targets, const std::vector<std::string> &noTargets, const bool &targetBoss, const bool &scanAll, const float &speed, const uint32_t &delay)
    {
        ...
        while (functionRunMap[{pid, "FollowTarget"}].load())
        {
            ...
            if (scanAll)
            {
                ...
                lastDist = 9999;
                std::cout << "\n======================================= spiralNum: " << goSpiral << " =======================================\n";
                std::cout << "backtrack count: " << backtrackCnt << '\n';
                if (goSpiral != 0 || backtrackCnt + 1 <= offsetBacktrack) {
                    GetNextPointSpiral(
                    targetX = game.data.player.data.coord.data.x.UpdateData().data,
                    targetZ = game.data.player.data.coord.data.z.UpdateData().data, visitedPoints, goSpiral >= offsetSpiral, (goSpiral == 0 && backtrackCnt + 1 <= offsetBacktrack));
                    if (goSpiral == 0 && backtrackCnt + 1 <= offsetBacktrack) {
                        backtrackCnt++;
    ...
    void GetNextPointSpiral(float &x, float &z,
                        ...
                        bool goBacktrack)
    {
        std::cout << (recenter ? "recenter" : "not recenter") << '\n';
        ...
        const double RING_GROWTH = 1.8;   // hệ số tăng bán kính hiệu dụng (>1.0)
        ...
        // ===== TRẠNG THÁI XOẮN (theo tâm hiện tại) =====
        struct SpiralState {
            bool init = false;
            int  ic = 0, jc = 0;   // tâm hiện tại
            int  ring = 1;         // ring hiện tại (logic)
            int  side = 0;         // cạnh 0..5
            ...
            int  ringMax = 1;      // RING CAO NHẤT đã đạt tại tâm này (để ghi/khôi phục)
            int  prevREff = 0;     // rEff của ring trước đó (để đảm bảo tăng đơn điệu nếu cần)
        };
        static SpiralState S;

        // ===== LỊCH SỬ TÂM (để backtrack) =====
        struct CenterState {
            int ic = 0, jc = 0;
            int ringMax = 1;   // ring cao nhất đã đạt ở tâm này
            int ring = 1;      // ring hiện tại khi lưu
            int side = 0, step = 0;
            bool atCorner = true;
            int prevREff = 0;
            int ci = 0, cj = 0;
        };
        static std::vector<CenterState> history; // stack các tâm đã qua
        static int curIdx = -1;                  // index của tâm hiện tại trong history

        auto saveCurrentCenterToHistory = [&](){
            if (curIdx >= 0 && curIdx < (int)history.size()) {
                history[curIdx].ic = S.ic; history[curIdx].jc = S.jc;
                history[curIdx].ringMax = std::max(history[curIdx].ringMax, S.ringMax);
                history[curIdx].ring    = S.ring;
                history[curIdx].side    = S.side;
                history[curIdx].step    = S.step;
                history[curIdx].atCorner= S.atCorner;
                history[curIdx].prevREff= S.prevREff;
                history[curIdx].ci      = S.ci;
                history[curIdx].cj      = S.cj;
            }
        };

        auto loadCenterFromHistory = [&](int idx){
            curIdx = idx;
            const auto &C = history[curIdx];
            S.ic = C.ic; S.jc = C.jc;
            S.ringMax = C.ringMax;
            // “tiếp tục với tham số ring cao nhất tại điểm đó”:
            S.ring = std::max(1, std::min(C.ringMax, R));
            S.side = C.side;  // bạn có thể reset về 0 nếu muốn bắt đầu lại vòng ringMax
            S.step = C.step;
            S.atCorner = true; // phát lại góc của ring hiện tại cho dễ đọc pattern
            S.prevREff = C.prevREff;
            S.ci = C.ci; S.cj = C.cj;
            S.init = true;
        };
        ...
        // ===== KHỞI TẠO BAN ĐẦU =====
        if (!S.init && history.empty()) {
            S.init = true;
            S.ic = i_cur; S.jc = j_cur;
            S.ring = 1; S.side = 0; S.step = 0; S.atCorner = true;
            S.ringMax = 1; S.prevREff = 0;
            CenterState C;
            C.ic = S.ic; C.jc = S.jc; C.ringMax = S.ringMax; C.ring = S.ring;
            C.side = S.side; C.step = S.step; C.atCorner = S.atCorner; C.prevREff = S.prevREff;
            C.ci = S.ci; C.cj = S.cj;
            history.push_back(C);
            curIdx = (int)history.size()-1;
        }

        // ===== XỬ LÝ BACKTRACK =====
        if (goBacktrack && !history.empty()) {
            // Lưu lại trạng thái tâm hiện tại trước khi rời đi
            if (curIdx >= 0) saveCurrentCenterToHistory();
            // Quay về tâm trước đó (nếu có)
            if (curIdx > 0) {
                loadCenterFromHistory(curIdx - 1);
            } else if ((int)history.size() >= 2) {
                loadCenterFromHistory((int)history.size()-2); // dự phòng
            } else {
                // chỉ có 1 tâm -> không làm gì
            }
        }

        // ===== XỬ LÝ RECENTER (đặt tâm mới tại vị trí hiện tại) =====
        if (recenter) {
            // lưu lại tâm cũ trước khi chuyển
            if (curIdx >= 0) saveCurrentCenterToHistory();

            // tìm xem tâm (i_cur,j_cur) đã có trong lịch sử chưa
            int found = -1;
            for (int i = 0; i < (int)history.size(); ++i) {
                if (history[i].ic == i_cur && history[i].jc == j_cur) { found = i; break; }
            }
            if (found >= 0) {
                loadCenterFromHistory(found);
            } else {
                // tạo tâm mới
                S.init = true;
                S.ic = i_cur; S.jc = j_cur;
        ...
        // đánh dấu ô hiện tại (tùy bạn có cần hay không)
        ...
        auto advance_one = [&](){
            ...
            if (S.atCorner) {
                // Góc mở đầu ở hướng NW của ring hiện tại (pattern chuẩn)
            ...
            if (S.step >= rEff) {
                ...
                if (S.side == 0) {
                    // hoàn tất 1 ring
                    S.ringMax = std::max(S.ringMax, S.ring);
                    S.prevREff = rEff;
                    S.ring++;
                    // Hành vi ở biên: bò mép (giữ rEff <= R)
                    if (ringEff(S.ring) > R) {
                        S.ring = S.ring; // giữ nguyên giá trị logic
                        S.prevREff = R;
                    }
                    S.atCorner = true;
                }
        ...
        // ===== GHI LẠI TRẠNG THÁI TÂM HIỆN TẠI VÀO LỊCH SỬ =====
        if (curIdx >= 0 && curIdx < (int)history.size()) {
            saveCurrentCenterToHistory();
        }
    }
