...
inline pair<float,float> Module::T1(float x, float y, int lastBossID) {
    ...
    static float exploreAngle = 0;
    static int idleCount = 0;
    static vector<pair<float, float>> boss36;

    const float STEP = 100.0f;
    const float RADIUS = 150.0f;
    const int DETECT_THRESHOLD = 3;
    if (lastBossID == 36) {
        boss36.push_back({x, y});
        idleCount = 0;
        if ((int)boss36.size() >= DETECT_THRESHOLD) {
            float sumX = 0, sumY = 0;
            for (auto &p : boss36) {
                sumX += p.first;
                sumY += p.second;
    ...
    if (mode == 1) {
        ...
        float nx = cx + RADIUS * cos(angle);
        float ny = cy + RADIUS * sin(angle);
        return {nx, ny};
    }

    exploreAngle += ((rand() % 30) - 15) * M_PI / 180.0f;
    float nx = x + STEP * cos(exploreAngle);
    float ny = y + STEP * sin(exploreAngle);
