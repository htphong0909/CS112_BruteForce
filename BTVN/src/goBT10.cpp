...
inline pair<float,float> Module::T1(float x, float y, int lastBossID) {
    static float centerX = x, centerY = y;  // lưu gốc xoắn ốc ban đầu
    ...
    if (lastBossID == 36) {
        ...
        centerY = y;
        foundSpecial = true;
        step = 0; // reset lại khi đổi mode
    }

    if (foundSpecial) {
        // Quay quanh boss đặc biệt
        float angle = (step % 360) * M_PI / 180.0f;
        float radius = 150 + 20 * sin(step / 15.0f);
        step = (step + 5) % 360;
        return {centerX + radius * cos(angle), centerY + radius * sin(angle)};
    } else {
        // Di chuyển khám phá theo xoắn ốc mở rộng
        float angle = step * 0.15f;
        float radius = 100 + step * 5;  // mở rộng đều ra ngoài
        step++;
        return {centerX + radius * cos(angle), centerY + radius * sin(angle)};
    }
}
