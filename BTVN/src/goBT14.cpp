...
inline pair<float,float> Module::T1(float x, float y, int lastBossID) {
	static std::pair<double, double> specialAreaCenter = {0.0, 0.0};
    static bool inSpecialArea = false;

    if (lastBossID == 36) {
        if (inSpecialArea == true) {
            specialAreaCenter.first = (specialAreaCenter.first * 0.7 + x * 0.3);
            specialAreaCenter.second = (specialAreaCenter.second * 0.7 + y * 0.3);
        }
        else specialAreaCenter = {x, y};
        inSpecialArea = true;
    }
    ...
    if (inSpecialArea) {
        double towardFactor = randDouble(0.3, 0.7);
        double jitterX = randDouble(-50, 50);
        double jitterY = randDouble(-50, 50);

        next_x = x + (specialAreaCenter.first - x) * towardFactor + jitterX;
        next_y = y + (specialAreaCenter.second - y) * towardFactor + jitterY;
    }
    else {
        next_x = x + randDouble(-100, 100);
        next_y = y + randDouble(-100, 100);
    }
