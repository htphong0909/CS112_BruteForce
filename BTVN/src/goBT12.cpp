...
// --- Các biến trạng thái toàn cục hoặc trong class của nhân vật ---
string state = "EXPLORING";
pair<float, float> last_special_zone_pos = {0, 0};
float exploration_angle = 0.0;
float exploitation_spiral_angle = 0.0;
float exploitation_spiral_radius = 100.0; // Bán kính khởi đầu, lớn hơn tầm quét
const float EXPLORATION_STEP = 150.0; // Khoảng cách mỗi bước đi khi thăm dò
const float SPIRAL_STEP_DISTANCE = 100.0; // Khoảng cách giữa các vòng xoắn ốc

// --- Hàm cần cài đặt ---
pair<float, float> getNextPoint(float x, float y, int lastBossID) {

    // 1. Cập nhật trạng thái dựa vào lastBossID
    if (lastBossID == 36) {
        state = "EXPLOITING"; // Đã tìm thấy khu vực đặc biệt!
        last_special_zone_pos = {x, y}; // Cập nhật "mỏ neo"
        // Reset lại vòng xoắn ốc để bắt đầu khai thác từ vị trí mới
        exploitation_spiral_angle = 0.0;
        exploitation_spiral_radius = SPIRAL_STEP_DISTANCE;
    }

    // 2. Xác định bước đi tiếp theo dựa vào trạng thái
    if (state == "EXPLOITING") {
        // --- Chế độ KHAI THÁC: Di chuyển theo hình xoắn ốc ---
        // Mục tiêu: Quét sạch khu vực xung quanh "mỏ neo" một cách có hệ thống.
        
        // Tính toán vị trí tiếp theo trên đường xoắn ốc Archimedes
        // Cứ mỗi vòng, bán kính sẽ tăng lên
        exploitation_spiral_angle += 1.0; // Tăng góc để di chuyển trên vòng tròn
        if (exploitation_spiral_angle > 2 * M_PI) { // Hoàn thành 1 vòng
            exploitation_spiral_angle = 0.0;
            exploitation_spiral_radius += SPIRAL_STEP_DISTANCE; // Tăng bán kính cho vòng tiếp theo
        }
        
        float next_x = last_special_zone_pos.first + exploitation_spiral_radius * cos(exploitation_spiral_angle);
        float next_y = last_special_zone_pos.second + exploitation_spiral_radius * sin(exploitation_spiral_angle);

        return {next_x, next_y};

    } else { // state == "EXPLORING"
        // --- Chế độ THĂM DÒ: Di chuyển theo một hướng cố định với nhiễu loạn nhỏ ---
        // Mục tiêu: Đi xa và khám phá bản đồ một cách hiệu quả.
        
        // Thêm một chút ngẫu nhiên vào hướng đi để tránh bị kẹt
        if (rand() % 10 == 0) { // Tỉ lệ 10% thay đổi hướng một chút
             exploration_angle += (rand() % 10 - 5) * (M_PI / 180.0); // Thay đổi ngẫu nhiên trong khoảng [-5, 5] độ
        }

        // Di chuyển một bước dài theo hướng đã định
        float next_x = x + EXPLORATION_STEP * cos(exploration_angle);
        float next_y = y + EXPLORATION_STEP * sin(exploration_angle);
        
        // Nếu không tìm thấy gì trong một thời gian dài (có thể thêm logic đếm), có thể thay đổi hướng lớn hơn
