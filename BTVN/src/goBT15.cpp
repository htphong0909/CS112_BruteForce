...
inline pair<float,float> Module::T1(float x, float y, int lastBossID) {
	static constexpr float STEP_LEN = 100.0f;
    static constexpr float REJOIN_THRESHOLD = 1.0f;
    static bool initialized = false;
    static float centerX = 0.0f, centerY = 0.0f;
    static float hx = 0.0f, hy = 0.0f;
    static int ring = 1;
    static int side = 0;
    static int stepOnSide = 0;
    static int stepsInSide = 1;
    static bool saw36 = false;
    if (!initialized) {
        initialized = true;
        centerX = x; centerY = y;
        hx = centerX; hy = centerY;
        ring = 1; side = 0; stepOnSide = 0; stepsInSide = 1;
        saw36 = false;
    }
    const float PI = 3.14159265358979323846f;
    static const float dirX[6] = {
        STEP_LEN * std::cosf(0.0f),
        STEP_LEN * std::cosf(PI/3.0f),
        STEP_LEN * std::cosf(2.0f*PI/3.0f),
        STEP_LEN * std::cosf(PI),
        STEP_LEN * std::cosf(4.0f*PI/3.0f),
        STEP_LEN * std::cosf(5.0f*PI/3.0f)
    };
    static const float dirY[6] = {
        STEP_LEN * std::sinf(0.0f),
        STEP_LEN * std::sinf(PI/3.0f),
        STEP_LEN * std::sinf(2.0f*PI/3.0f),
        STEP_LEN * std::sinf(PI),
        STEP_LEN * std::sinf(4.0f*PI/3.0f),
        STEP_LEN * std::sinf(5.0f*PI/3.0f)
    };
    ...
    if (dist(x, y, hx, hy) > REJOIN_THRESHOLD) return {hx, hy};
    hx += dirX[side];
    hy += dirY[side];
    stepOnSide++;
    if (stepOnSide >= stepsInSide) {
        stepOnSide = 0;
        side++;
        if (side >= 6) {
            side = 0;
