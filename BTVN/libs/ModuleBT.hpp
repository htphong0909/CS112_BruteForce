...
// (Tùy chọn) I/O kiểu C nếu cần tốc độ/định dạng đặc thù
#include <cstdio>
...
namespace Module
{
    ...
    static uint32_t offsetSpiral = 1;
    ...
    inline std::pair<float, float> T1(float x, float y, int lastBossID);
    void GetNextPoint(float &x, float &z, std::unordered_set<std::string> &visitedPoints);
    void GetNextPointSpiral(float &x, float &z, std::unordered_set<std::string> &visitedPoints, int lastBossID = 0, bool goBacktrack = false);
...
namespace Module
{
        ...
        while (functionRunMap[{pid, "FollowTarget"}].load())
        {
            ...
            if (scanAll)
            {
                ...
                GetNextPointSpiral(
                    ...
                    targetZ = game.data.player.data.coord.data.z.UpdateData().data, visitedPoints, lastBossID);
    ...
    std::unique_ptr<Game::World::Entity> FindTarget(Game &game, const bool &targetBoss, const bool &targetPlant, const bool &targetNormal, const std::vector<std::string> &targets, const std::vector<std::string> &noTargets, const uint32_t &range, const std::vector<Target> &blackTargetList)
    {
        ...
        for (auto &entity : entitys)
        {
            ...
            if (std::regex_match(currentTarget.name, std::regex(".*chest.*"))) continue;
    ...
    {
        std::cout << "INPUT:\n";
        std::cout << std::fixed << "x = " << x << " y = " << z << " lastBossID = " << lastBossID << "\n";

        std::cerr << "INPUT:\n";
        std::cerr << std::fixed << x << ", " << z << ", " << lastBossID << "\n";

        std::pair<float, float> newData = T1(x, z, lastBossID);
        x = newData.first;
        z = newData.second;
        std::cout << "OUTPUT:\n";
        std::cout << std::fixed << "x = " << x << " y = " << z << "\n";

        std::cerr << "OUTPUT:\n";
        std::cerr << std::fixed << x << ", " << z << "\n";
    }
